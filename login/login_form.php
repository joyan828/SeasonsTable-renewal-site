...
<head>
    ...
    <![endif]-->
    <script>
        //로그인 창에 접속 시 로그인 input에 focus 주기
...
<body>
        ...
        <div id="login_form">
            ...
            <div id="login_notmem" class="memchk">
                <form name="ordercheck" action="" method="post">
                    ...
                    <!-- login_btn -->
                    <div class="login_btn">
                        <input type="button" class="l_btn" value="주문번호확인">
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        $('#login_tab li').on("click", function() {
            ...
            $('#' + tabId).find('input:text').eq(0).focus(); //ie 확인
            $('#' + tabId).find('input:not(:button)').val("");
            $(".error_msg").hide();
        });
        ...
        $(".l_btn").on("click", function() {
            ...
            //input 입력값이 없으면 각각 아이디, 비밀번호를 입력하게 하는 알림메시지를 보인다.
                ...
                if (!txt.eq(i).val()) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response == 1) { //로그인 통과
                                $('#loginerr').hide();
                                location.href = '../index.php'; //메인페이지로 이동
                            } else if (response == 0) { //로그인 통과하지 못할 시 에러 메시지 띄우기
                                $('#loginerr').show();
                            }
                        }
                    }); //ajax
